...
text = restore_text(root)
print(text)  # Pass parsed XML element

def get_words(text):
    return text.split()

word = get_words(text)
print(word) # ['Τῇδε', 'πᾶς', 'ἕπου,', 'δίωκε', 'καὶ', 'τὸν', 'ἄνδρα', 'πυνθάνου']

#def get_words_xml(word):
#    '''
#    text = '<l n="204" metre="4 tr^" speaker="ΧΟ."><syll weight="heavy">Τῇ</syll><syll weight="light">δε</syll> <syll weight="heavy">πᾶ</syll><syll weight="light" anceps="True">ς ἕ</syll><syll weight="heavy">που</syll>, </l>'
#    >>> print(get_words_xml(text))
#    >>> ['<syll weight="heavy">Τῇ</syll><syll weight="light">δε</syll>', '<syll weight="heavy">πᾶ</syll>', '<syll weight="light" anceps="True">ς ἕ</syll><syll weight="heavy">που</syll>']
#    '''
#    pass
